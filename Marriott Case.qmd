---
title: "Marriott"
author: "Aaron Murdoch, Brandon Bath, & Tyler O'Keefe"
format: pdf
editor: visual
---
...
Build a tsibble (convert date to date type, make it a tsibble to use forecasting model tools -instead of a tibble) (got rid of the dow indicator variable, don't need it?
...
final_models2 <- train_valid %>%
  model(RegDow = TSLM(pickup_ratio~dow))

augment(final_models2)
```

Forecast on Test

```{r}
ratio_fc_test1 <- final_models1 %>%
  forecast(new_data = test)

forecasted_demand1 <- ratio_fc_test1 %>%
